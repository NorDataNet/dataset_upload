...
class DatasetUploadForm extends DatasetValidationForm {
   ...
   * @param array $form
   *   The form object.
   * @param \Drupa\Form\FormStateInterface $form_state
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if ($form_state->has('nird_error')) {
      ...
      $key = 'nird_api_error';
      $to = 'magnarem@met.no';
      $params['message'] = $this->t('Something went wrong registering the dataset\n The NIRD API JSON request was:\n @json \n and the NIRD API response was:\n @nird',
        [
          '@json' => $form_state->get('json'),
          '@nird' => $form_state->get('nird_error'),
        ]);
  ...
  public function buildDatasetForm(array $form, FormStateInterface $form_state) {
    ...
    /*
     * Load the current user object to extract more information about the user.
     */
    $user = User::load($this->currentUser->id());
...
confirming your submission. If the metadata are not correct, cancel your submission, correct your information and proceed with a new submission.</span>',
    ...
    $date = $metadata['date_created'] ?? date(DATE_ISO8601);
    $date_format = 'Y-m-d';
    $time_format = 'H:i:s';
    ...
    for ($i = 0; $i < $max; $i++) {
      ...
      elseif (strtolower($depositor_role[$i]) === strtolower('Data Manager') && ($depositor_type[$i] === 'institution' || $depositor_type[$i] === 'organization')) {
        if (isset($depositor_name[$i])) {
          ...
          $shortname = $matches[0][1] ?? $matches[1][0];
    ...
    /* TODO: add data manager organization form datamanger role are org s*/
    /*
    ...
    $domain = $form_state->getValue(['dataset', 'subject', 'domain']);
    $field = $form_state->getValue(['dataset', 'subject', 'field']);
    $subfield = $form_state->getValue(['dataset', 'subject', 'subfield']);
    ...
    if ($subjects_added !== NULL) {
      ...
      for ($i = 0; $i < $num_subjects; $i++) {
        ...
        $form['dataset']['subject']['subjects'][$i] = [
          '#type' => 'item',
          '#title' => $this->t('Subject # @num', ['@num' => $num]),
    ...
    if (!empty($field) && !empty($domain) && !empty($subfield)) {
      ...
      $form['dataset']['subject']['actions']['addsubject'] = [
        '#type' => 'submit',
  ...
  public function validate(array &$form, FormStateInterface $form_state) {
    ...
    // Do not delete uploaded dataset after compliance checker validation.
    $form_state->set('keep_file', 1);
    // Override the tests to be used in compliance checker.
    $form_state->set('tests_override', ['cf' => 'cf:1.6', 'acdd' => 'acdd']);
      ...
      if ($st) {
        ...
        $form['register']['manager']['registerdm']['longname'] = [
          ...
          '#title' => $this
            ->t('Long name'),
        // Extract from metadata.
          '#default_value' => $dataset['data_manager'][0]['manager']['longname'],
        // '#required' => true,
        // '#disabled' => true,.
          '#states' => [
            'invisible' => [
        ...
        $form['register']['manager']['registerdm']['shortname'] = [
          ...
          '#title' => $this
            ->t('Short name'),
        // Extract from metadata.
          '#default_value' => $dataset['data_manager'][0]['manager']['shortname'],
           // '#required' => true,
           // '#disabled' => true,.
        ...
        $form['register']['manager']['registerdm']['contactemail'] = [
          ...
          '#title' => $this
            ->t('Contact email'),
        // Extract from metadata.
          '#default_value' => $dataset['data_manager'][0]['manager']['contactemail'],
               // '#required' => true,
               // '#disabled' => true,.
        ...
        $form['register']['manager']['registerdm']['homepage'] = [
          ...
          '#title' => $this
            ->t('Homepage'),
        // Extract from metadata.
          '#default_value' => $dataset['data_manager'][0]['manager']['homepage'],
                   // '#required' => true,
                   // '#disabled' => true,.
      ...
      if ($st) {
        ...
        $form['register']['holder']['registerrh']['longname'] = [
          ...
          '#title' => $this
            ->t('Long name'),
        // Extract from metadata.
          '#default_value' => $dataset['rights_holder']['holder']['longname'],
        // '#required' => true,
        // '#disabled' => true,.
        ...
        $form['register']['holder']['registerrh']['shortname'] = [
          ...
          '#title' => $this
            ->t('Short name'),
        // Extract from metadata.
          '#default_value' => $dataset['rights_holder']['holder']['shortname'],
           // '#required' => true,
           // '#disabled' => true,.
        ...
        $form['register']['holder']['registerrh']['contactemail'] = [
          ...
          '#title' => $this
            ->t('Contact email'),
        // Extract from metadata.
          '#default_value' => $dataset['rights_holder']['holder']['contactemail'],
               // '#required' => true,
               // '#disabled' => true,.
        ...
        $form['register']['holder']['registerrh']['homepage'] = [
          ...
          '#title' => $this
            ->t('Homepage'),
        // Extract from metadata.
          '#default_value' => $dataset['rights_holder']['holder']['homepage'],
                   // '#required' => true,
                   // '#disabled' => true,.
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (isset($result['dataset_id'])) {
      ...
      if ($mime_type === 'application/x-netcdf') {
        ...
        // Delete the uploaded archive after extraction.
        $file->delete();
      }
      ...
      file_put_contents($yaml_filepath, $yml, FileSystemInterface::EXISTS_REPLACE);
  ...
  public function cancelSubmission(array &$form, FormStateInterface $form_state) {
    $this->messenger()->addMessage($this->t("Cancel submission (clear all data)"));
  ...
  public function addSubjectCallback(array &$form, FormStateInterface $form_state) {
    ...
    return $response;
     */
    $form_state->unsetValue(['dataset', 'subject', 'domain']);
    $form_state->unsetValue(['dataset', 'subject', 'field']);
    $form_state->unsetValue(['dataset', 'subject', 'subfield']);
  ...
  /**
   * Add Depositor person ajax function.
   */
  ...
  /**
   * Remove depositor person ajax function.
   */
  ...
  /**
   * Add depositor organization ajax function.
   */
  ...
  /**
   * Remove depositor org ajax function.
   */
